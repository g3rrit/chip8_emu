...
#define INVALID_INSTRUCTION() panic("Invalid instruction [call] [%04x]", op)

    int draw_flag = 0;

    if (cpu.pc + 1 >= MEM_SIZE) {
        panic("Invalid memory access [%04x]", cpu.pc);
    }
    ...
    switch (GETO(3)) {
        ...
        case 8:
            switch (GETO(0)) {
                ...
                case 5: {
                    uint16_t r = cpu.v[GETO(2)] - cpu.v[GETO(1)];
                    cpu.v[0xf] = (cpu.v[GETO(2)] > cpu.v[GETO(1)]);
                    cpu.v[GETO(2)] = r & 0xff;
                    break;
                }
                case 6:
                ...
                case 7: {
                    ...
                    cpu.v[0xf] = (cpu.v[GETO(1)] > cpu.v[GETO(2)]);
                    cpu.v[GETO(2)] = r & 0xff;
                    break;
                }
                case 0xe:
        ...
        case 0xc:
            ...
            break;
        case 0xd:
            draw_sprite(cpu.v[GETO(2)] % SCREEN_WIDTH, cpu.v[GETO(1)] % SCREEN_HEIGHT, GETO(0));
            draw_flag = 1;
            break;
        case 0xe:
            if ((op & 0x00ff) == 0x9e) {
